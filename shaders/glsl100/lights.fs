...
{
    float height = 1.0 - texture2D(materials[HEIGHT].texture, uv).r;
...
vec2 DeepParallax(vec2 uv, vec3 V)
{
    ...
    vec2 currentUV = uv;
    float currentDepthMapValue = 1.0 - texture2D(materials[HEIGHT].texture, currentUV).y;
    
    while(currentLayerDepth < currentDepthMapValue)
    {
        currentUV += deltaTexCoord;
        currentLayerDepth += layerDepth;
        currentDepthMapValue = 1.0 - texture2D(materials[HEIGHT].texture, currentUV).y;
    }

    vec2 prevTexCoord = currentUV - deltaTexCoord;
    float afterDepth  = currentDepthMapValue + currentLayerDepth;
    float beforeDepth = 1.0 - texture2D(materials[HEIGHT].texture,
...
void main()
{
    ...
    albedo *= texture2D(materials[ALBEDO].texture, uv).rgb*float(materials[ALBEDO].active);
    ...
    if (materials[METALNESS].active != 0)
        metalness *= texture2D(materials[METALNESS].texture, uv).b;
    ...
    if (materials[ROUGHNESS].active != 0)
        roughness *= texture2D(materials[ROUGHNESS].texture, uv).g;
    ...
    vec3 N = (materials[NORMAL].active == 0) ? normalize(fragNormal)
        : normalize(TBN*(texture2D(materials[NORMAL].texture, uv).rgb*2.0 - 1.0));
    ...
    {
        float ao = texture2D(materials[OCCLUSION].texture, uv).r;
        ambient *= ao;
    ...
    {
        emission *= texture2D(materials[EMISSION].texture, uv).rgb;
    }
